...
// tag::txn_api[]
result_t db_create(const char *filename, db_options_t *options, db_t *db);
...
static inline void bitmap_set(uint64_t *buffer, uint64_t pos, bool val) {
    if (val)
...
result_t hash_get_next(txn_t *tx, pages_map_t **state, hash_val_t *it);
result_t hash_get_entries_count(txn_t *tx, uint64_t hash_id,
                                uint64_t *number_of_entries);
// end::hash_api[]

// tag::hash_multi_api[]
result_t hash_multi_append(txn_t *tx, hash_val_t *set, uint64_t container_id);
result_t hash_multi_del(txn_t *tx, hash_val_t *del, uint64_t container_id);
result_t hash_multi_get_next(txn_t *tx, pages_map_t **state, hash_val_t *it,
                             uint64_t container_id);
// end::hash_multi_api[]
...
result_t table_get_schema(txn_t *tx, char *table_name, table_schema_t *schema);
