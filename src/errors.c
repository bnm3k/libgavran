...
#include "../include/gavran/infrastructure.h"
...
__attribute__((__format__(__printf__, 4, 5))) static bool try_sprintf(
    char** buffer, char* buffer_end, size_t* chars, const char* format, ...) {
    va_list ap;
    va_start(ap, format);
...
// tag::errors_push_new[]
op_result_t* errors_push_new(const char* file,
                             uint32_t line,
                             const char* func,
                             int32_t code) {
    // <1>
...
size_t errors_get_count() {
    return _errors_count;
}

uint32_t errors_get_oom_flag() {
